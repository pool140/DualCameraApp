...
class CameraRecordingService : Service() {
    ...
    override fun onCreate() {
        ...
        sendBroadcast(Intent(ACTION_STATUS).apply { putExtra(EXTRA_CONCURRENT, false) })
            ...
            try {
                ...
                mainHandler.postDelayed({
                    try {
                        ...
                        Log.e(TAG, "front delayed: " + e.message)
                    }
                }, 2000)
                isRecording = true
                updateNotification("يسجل - اخفض الصوت للايقاف")
                sendBroadcast(Intent(ACTION_STATUS).apply { putExtra(EXTRA_STATUS, "recording") })
            } catch (e: Exception) {
    ...
    private fun stopRecording() {
        backgroundHandler.post {
            try {
                ...
                sendBroadcast(Intent(ACTION_STATUS).apply { putExtra(EXTRA_STATUS, "stopped") })
            }
        }
    }
    ...
    private fun openCamera(id: String, recorder: MediaRecorder, isFront: Boolean) {
        try {
                ...
                override fun onOpened(cam: CameraDevice) {
                    ...
                    cam.createCaptureSession(listOf(recorder.surface), object : CameraCaptureSession.StateCallback() {
                        override fun onConfigured(s: CameraCaptureSession) {
                            ...
                            try {
                                s.setRepeatingRequest(b.build(), null, backgroundHandler)
                                recorder.start()
                            } catch (e: Exception) {
                                Log.e(TAG, "session err: " + e.message)
                            }
                        }
            ...
            Log.e(TAG, "open camera err: " + e.message)
        }
    }
